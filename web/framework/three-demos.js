...
function orbitalLobe(color, opacity = 0.32) {
  const geometry = new THREE.SphereGeometry(0.72, 32, 20);
...
function atomOrbital() {
  ...
  const left = orbitalLobe(INK, 0.28);
  left.position.x = -0.72;
  group.add(left);

  const right = orbitalLobe(0xffffff, 0.5);
...
export function orbitalOverlap(host) {
  ...
  overlap.scale.set(0.4, 0.58, 0.58);
  pair.add(overlap);

  ctx.camera.position.set(0, 1.25, 6.8);
  ctx.camera.lookAt(0, 0, 0);

  const update = () => {
    const p = Number(ctx.range.value) / 100;
    const separation = 4.1 - p * 2.15;
    leftAtom.position.x = -separation / 2;
    rightAtom.position.x = separation / 2;
    const overlapStrength = Math.max(0, Math.min(1, (3.15 - separation) / 1.2));
    overlap.scale.x = 0.12 + overlapStrength * 0.95;
    overlap.material.opacity = 0.04 + overlapStrength * 0.28;
...
export function explodeAssemble(host) {
  ...
  assembly.rotation.set(-0.22, 0.48, 0.04);
  ctx.scene.add(assembly);

  const assembled = [
    [-0.75, 0.42, 0], [0, 0.42, 0], [0.75, 0.42, 0],
    [-0.75, -0.38, 0], [0, -0.38, 0], [0.75, -0.38, 0],
  ];
  const exploded = [
    [-2.0, 1.2, 0.8], [0, 1.75, -0.45], [2.15, 1.05, 0.55],
    [-2.1, -1.0, -0.55], [0, -1.65, 0.75], [2.0, -1.08, -0.72],
  ];

  const pieces = assembled.map((position, index) => {
    const piece = outlinedBox(0.68, 0.68, 0.68, index === 2 ? ACCENT : 0xffffff);
    piece.position.fromArray(exploded[index]);
    assembly.add(piece);
    return piece;
  });

  ctx.camera.position.set(0.2, 1.45, 6.8);
  ctx.camera.lookAt(0, 0, 0);
...
export function anchoredCallout(host) {
  ...
  const object = outlinedBox(2.0, 1.45, 1.25, 0xffffff);
  ctx.scene.add(object);

  const localAnchor = new THREE.Vector3(1.0, 0.72, 0.62);
  ...
  ctx.camera.position.set(3.4, 2.4, 5.3);
  ctx.camera.lookAt(0, 0, 0);
