...
## Two parallel projections and machine selection

Web and machine retrieval are actor-neutral views over the same canonical Core, but they do not mirror each other's interaction workflow. Default Human navigation is **Atlas | Explore | Dictionary | Index**. The Human Atlas defaults to the cross-media Creative World, grouped as an editorial browsing surface for visual recognition, understanding and comparison. Academic Subject Packs remain available through subject → canonical knowledge point → concrete examples without becoming the Human taxonomy. Explore discovers Examples through search plus secondary refinement by world, creative domain, Concept, medium, interaction, visual trait, Resource availability and coverage kind. Dictionary explains a Concept through real Examples and then exposes related Concepts plus Resource/Tool implementation paths. Index is the exhaustive compact entity browser. No curriculum-board navigation and no Human selection/cart/commit surface.

Human presentation is a derived projection over canonical fields and relations, not a new canonical content layer. A separate presentation audit under `app/media.json` may map an Example stable ID to an original-source embed, official remote preview, publisher-controlled on-demand frame or fallback plate. This audit carries display/provenance decisions only and never changes Example identity or canonical facts.

External REFERENCE, local LIVE and Resource demo remain distinct. Human rendering should prefer the actual original reference whenever an open-license, explicit embed or publisher-controlled remote display path exists; no third-party binary is copied into the repository without a clear redistribution basis. The medium-aware reference plate is the last fallback, not the default representation.

Example detail displays identity first, then original/local visual media, concise summary and Concepts, reuse/failure notes, related Examples and Resources, with technical/provenance material collapsed. Machine/build projection is not a Human UI section. Opening an Example may fetch its compact resolve record, but never full Resource artifacts. Initial startup loads only app HTML/CSS/JS and web-index; AI index, relations, canonical records and build data are lazy.

Machine retrieval uses `query → stable-ID resolve/compare → explicit selection commit → AI lock → deep fetch`. Commit carries exact Example IDs, per-ID aspect notes and constraints. Lock preserves them verbatim, records the catalog fingerprint and refuses changed IDs, changed notes, uncommitted input, stale catalogs or GAP substitution. A lock is a local integrity contract, not an authentication signature. Deep fetch requires a validated lock, reads only declared package artifacts, and does not fetch arbitrary URLs. Catalog edits after lock require an explicit new commit/lock. Existing selector fields remain a machine-protocol compatibility detail; the Human projection does not create, edit, commit or download selection objects. Stable URLs and IDs are sufficient for a person to point an agent at an exact reference.
...
Validate schemas/IDs, type-directed relations, provenance, topic cycles and inventory, crosswalk scope/rationale, adapter completeness, resource paths, generated drift, FTS retrieval and identity-preserving machine lock/deep fetch (including rejection tests). Browser tests exercise all four Human views, actual interactions, absence of Human selection UI, mobile overflow, collapsed details, explicit GAPs, startup requests and console errors. Capture and visually inspect desktop/mobile screenshots. CI validates this contract and retains legacy **data** validators; old board UI/route validators are retired from required CI.
