...
Human presentation is a derived projection over canonical fields and relations, not a new editable content layer. External REFERENCE, local LIVE and Resource demo are distinct: external works use an honest medium-aware reference fallback plus source link unless provenance explicitly establishes a lawful display/embed mode; LIVE uses the local implementation; Resource pages may run declared original AIDRB demo artifacts and must state that the demo is not the source Example.

Example detail displays visual/reference first, brief identification second, then why it is banked, demonstrated Concepts, reusable observations, transfer/failure constraints, related Examples and Resources. Machine/build details and provenance remain accessible but subordinate/collapsed. Opening an Example may fetch its compact resolve record, but never full Resource artifacts. Initial startup loads only app HTML/CSS/JS and web-index; AI index, relations, canonical records and build data are lazy.
