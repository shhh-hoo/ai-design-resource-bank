...
def build(output=ROOT/'catalog', check=False):
    ...
    for r in records:
        ...
        keys=['id','type','kind','title','summary','aliases','use_when','avoid_when','subject_id','topic_ids','concept_ids','visual_traits','medium','interaction','intent_ids','preview','lifecycle','verification','resource_ids','tool_ids','canonical_path']
    ...
    outputs={'web-index.json':encoded(web),'relations.json':encoded(relations),'aliases.json':encoded(aliases),
             ...
             'ai-index.jsonl':b''.join((json.dumps(a,ensure_ascii=False,sort_keys=True)+'\n').encode() for a in ai)}
    fts=[]
    for a in ai:
        r=by_id[a['id']]
        context=[by_id[x]['title'] for x in r.get('topic_ids',[])+r.get('concept_ids',[])+r.get('intent_ids',[])]
    ...
    for r in records:
        result=dict(record=r,relations=[e for e in relations if r['id'] in (e['source'],e['target'])],
                    sources=[by_id[s] for s in r.get('source_refs',[])])
        if r['type']=='Example':
    ...
    legacy=dict(version=1,resources=[dict(id=r['legacy_id'],title=r['title'],domain=r['domain'],tags=r.get('tags',[]),
        path=r['package_path'],status=r['status'],hint=r['summary']) for r in records if r['type']=='Resource'])
    # JSON is YAML 1.2: legacy consumers retain their path without a second editable catalog.
    if check:
