...
from atlas_core import ROOT, TYPES, RELATIONS, inputs, generate_relations, read, validate_shapes, ID_PREFIX
...
def validate(records, aliases, migration, crosswalks):
    ...
    for r in records:
        ...
        if type=='Example':
            ...
            if topic_ids:
                require(subject_id in by_id and by_id.get(subject_id,{}).get('parent') is None,f'{id}: invalid subject')
                ...
                require(subject_id is None,f'{id}: subject_id requires topic_ids')
            if gap:
    ...
    for m in migration['curriculum_subjects']: require(all(s in by_id for s in m['source_ids']),'Curriculum migration dangling source')
