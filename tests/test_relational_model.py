...
class ModelTests(unittest.TestCase):
    def setUp(self):
        ...
        self.put('sources',id='src:fixture',source_key='synthetic-fixture',title='SYNTHETIC STRUCTURAL TEST ONLY',publisher='fixture',locator_type='other',locator='synthetic-fixture',captured_at='2026-09-13',check_scope='structural fixture')
        for i,k in [('proof:observed','observation'),('proof:reasoning','reasoning'),('proof:test','test')]:
            ...
            if k!='reasoning':self.put('evidence_set_sources',evidence_set_id=i,source_id='src:fixture',locator='fixture-only',stance='supports')
    ...
    def kp(self):
        self.named('subjects','subject:chemistry')
        self.named('subjects','subject:organic',parent_id='subject:chemistry')
        self.named('knowledge_points','kp:sn2')
        self.put('knowledge_point_subjects',knowledge_point_id='kp:sn2',subject_id='subject:organic',is_primary=1,evidence_set_id='proof:observed')
    ...
    def test_02_creative_reference_without_academic_or_resource(self):
        self.entity('example:curtain','example',origin_kind='real_reference')
        self.entity('concept:reveal','concept',definition='Synthetic definition',kinds_json='["effect"]')
        self.put('entity_relations',from_entity_id='example:curtain',to_entity_id='concept:reveal',predicate='demonstrates',evidence_set_id='proof:observed')
    ...
    def test_03_subject_ancestry(self):
        self.kp()
        rows=self.db.execute("WITH RECURSIVE ancestry(id) AS (SELECT subject_id FROM knowledge_point_subjects WHERE knowledge_point_id='kp:sn2' UNION SELECT s.parent_id FROM subjects s JOIN ancestry a ON s.id=a.id WHERE s.parent_id IS NOT NULL) SELECT id FROM ancestry ORDER BY id").fetchall()
        self.assertEqual([('subject:chemistry',),('subject:organic',)],rows)
    def test_04_shared_representation_identity(self):
        self.kp();self.named('knowledge_points','kp:another-topic')
        self.entity('concept:sequence','concept',definition='Synthetic method',kinds_json='["representation_method","technique"]')
        for kp in ('kp:sn2','kp:another-topic'):
            self.put('knowledge_point_representations',knowledge_point_id=kp,concept_id='concept:sequence',context_key='step-order',context_json='{"purpose":"show step order"}',verdict='conditional',rationale='Synthetic proposal',curator='fixture',evidence_set_id='proof:reasoning')
    ...
    def test_05_primitive_many_capabilities(self):
        ...
        for cap in ('cap:picking','cap:intersection'):
            ...
            self.put('primitive_capabilities',tool_primitive_id='primitive:picker',capability_id=cap,scope_key='fixture',evidence_set_id='proof:observed')
    ...
    def test_06_illegal_endpoint_insert_and_update(self):
        ...
        with self.assertRaises(sqlite3.IntegrityError):self.put('entity_relations',from_entity_id='tool:x',to_entity_id='concept:x',predicate='demonstrates',evidence_set_id='proof:observed')
        self.entity('example:x','example',origin_kind='authored_study')
        self.put('entity_relations',from_entity_id='example:x',to_entity_id='concept:x',predicate='demonstrates',evidence_set_id='proof:observed')
    ...
    def test_07_inverse_not_double_stored(self):
        self.entity('concept:x','concept',definition='x',kinds_json='["effect"]');self.entity('example:x','example',origin_kind='real_reference')
        with self.assertRaises(sqlite3.IntegrityError):self.put('entity_relations',from_entity_id='concept:x',to_entity_id='example:x',predicate='seen_as',evidence_set_id='proof:observed')
    def test_08_orphan_foreign_key(self):
        with self.assertRaises(sqlite3.IntegrityError):self.put('knowledge_point_subjects',knowledge_point_id='missing',subject_id='missing',evidence_set_id='proof:observed')
    ...
    def test_10_recommendation_not_fact(self):
        ...
        self.put('entity_recommendations',from_entity_id='concept:x',to_entity_id='tool:x',predicate='can_implement_with',context_key='web',context_json='{"purpose":"web effect"}',verdict='conditional',rationale='Only a proposal',curator='fixture',review_status='accepted',evidence_set_id='proof:reasoning')
    ...
    def test_11_nonmethod_rejected(self):
        ...
        with self.assertRaises(sqlite3.IntegrityError):self.put('knowledge_point_representations',knowledge_point_id='kp:sn2',concept_id='concept:style',context_key='x',context_json='{"purpose":"x"}',verdict='suitable',rationale='x',curator='fixture')
    def test_12_graph_cycles_rejected_by_build_validator(self):
        for x in ('kp:a','kp:b'):self.named('knowledge_points',x)
        for a,b in [('kp:a','kp:b'),('kp:b','kp:a')]:self.put('knowledge_point_relations',from_knowledge_point_id=a,to_knowledge_point_id=b,predicate='is_a',evidence_set_id='proof:observed')
        self.assertTrue(any('cycle' in e for e in validate(self.db)))
    def test_13_prerequisite_requires_scope(self):
        for x in ('kp:a','kp:b'):self.named('knowledge_points',x)
        with self.assertRaises(sqlite3.IntegrityError):self.put('knowledge_point_relations',from_knowledge_point_id='kp:a',to_knowledge_point_id='kp:b',predicate='prerequisite_for',evidence_set_id='proof:reasoning')
    def test_14_missing_evidence_locator_is_not_publishable(self):
        self.entity('example:x','example',origin_kind='real_reference');self.entity('concept:x','concept',definition='x',kinds_json='["effect"]')
        self.put('entity_relations',from_entity_id='example:x',to_entity_id='concept:x',predicate='demonstrates',evidence_set_id='proof:observed')
        self.assertEqual(1,self.db.execute('SELECT count(*) FROM fact_entity_relations').fetchone()[0])
        self.db.execute("DELETE FROM evidence_set_sources WHERE evidence_set_id='proof:observed'")
        self.assertEqual('unreviewed',self.db.execute("SELECT review_status FROM evidence_sets WHERE id='proof:observed'").fetchone()[0])
    ...
    def test_15_stale_resource_validation(self):
        self.entity('resource:x','resource',resource_type='code',package_path='resources/x',validation_status='passed',validated_hash='f'*64,validation_evidence_set_id='proof:test')
    ...
    def test_16_reference_and_resource_share_artifact(self):
        ...
        self.entity('example:x','example',origin_kind='authored_study',preview_artifact_id='artifact:x')
    ...
    def test_17_no_unconditional_scientific_suitability(self):
        ...
        self.put('knowledge_point_representations',knowledge_point_id='kp:sn2',concept_id='concept:x',context_key='missing',verdict='suitable',rationale='x',curator='fixture')
        self.assertTrue(any('purpose' in e for e in validate(self.db)))
    def test_18_reasoning_not_implementation_fact(self):
        self.entity('example:x','example',origin_kind='real_reference');self.entity('tool:x','tool',tool_subtype='library')
        self.put('entity_relations',from_entity_id='example:x',to_entity_id='tool:x',predicate='built_with',evidence_set_id='proof:reasoning')
