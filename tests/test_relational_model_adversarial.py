...
class Base(unittest.TestCase):
    def setUp(self):
        ...
        self.put('sources',id='src:manual',source_key='manual',title='Manual record',publisher='fixture',locator_type='other',locator='manual-record',captured_at='2026-09-13',check_scope='structural fixture')
        self.put('sources',id='src:web',source_key='web-v1',title='Web source',publisher='example.invalid',locator_type='url',locator='https://example.invalid',captured_at='2026-09-13',check_scope='structural fixture')
        for i,k,status in [('ev:obs','observation','unreviewed'),('ev:test','test','unreviewed'),('ev:reason','reasoning','accepted'),('ev:unreviewed','observation','unreviewed')]:
            self.put('evidence_sets',id=i,basis_kind=k,summary=i,producer='test',review_status=status,canonical_locator=f'evidence/{i}')
        self.put('evidence_set_sources',evidence_set_id='ev:obs',source_id='src:web',locator='section-1',stance='supports')
        self.put('evidence_set_sources',evidence_set_id='ev:test',source_id='src:manual',locator='test-log',stance='supports')
        self.db.execute("UPDATE evidence_sets SET review_status='accepted' WHERE id IN ('ev:obs','ev:test')")
...
class GoodCases(Base):
    def test_A_pure_creative_unknown_tool(self):
        self.entity('example:curtain','example',medium='stage-video')
        ...
        self.put('entity_domains',entity_id='example:curtain',creative_domain_id='domain:stage',is_primary=1,evidence_set_id='ev:obs')
        self.put('entity_relations',from_entity_id='example:curtain',to_entity_id='concept:occlusion',predicate='demonstrates',evidence_set_id='ev:obs')
    ...
    def test_B_editorial_prompt_resource_without_implementation_chain(self):
        ...
        self.put('entity_relations',from_entity_id='resource:prompt',to_entity_id='concept:editorial',predicate='implements',evidence_set_id='ev:obs')
        self.assertEqual([], validate(self.db))
        self.assertEqual(0,self.db.execute('SELECT count(*) FROM tool_capabilities').fetchone()[0])

    def test_C_diffusion_cross_subject(self):
        for s in ['subject:chem','subject:bio','subject:physics']:
            self.named('subjects',s)
        self.named('knowledge_points','kp:diffusion')
        self.put('knowledge_point_subjects',knowledge_point_id='kp:diffusion',subject_id='subject:chem',is_primary=1,evidence_set_id='ev:obs')
        self.put('knowledge_point_subjects',knowledge_point_id='kp:diffusion',subject_id='subject:bio',evidence_set_id='ev:obs')
        self.put('knowledge_point_subjects',knowledge_point_id='kp:diffusion',subject_id='subject:physics',evidence_set_id='ev:obs')
        self.assertEqual([], validate(self.db))
        self.assertEqual(3,self.db.execute("SELECT count(*) FROM knowledge_point_subjects WHERE knowledge_point_id='kp:diffusion'").fetchone()[0])

    def test_D_multi_parent_knowledge_graph(self):
        for k in ['kp:sn2','kp:nuc-sub','kp:mechanism']:
            self.named('knowledge_points',k)
        self.put('knowledge_point_relations',from_knowledge_point_id='kp:sn2',to_knowledge_point_id='kp:nuc-sub',predicate='is_a',scope_key='global',is_navigation_parent=1,evidence_set_id='ev:obs')
        self.put('knowledge_point_relations',from_knowledge_point_id='kp:sn2',to_knowledge_point_id='kp:mechanism',predicate='part_of',scope_key='global',evidence_set_id='ev:obs')
        self.assertEqual([], validate(self.db))

    def test_E_representation_concept_shared_across_subjects(self):
        for s in ['subject:chem','subject:physics']:
            self.named('subjects',s)
        for k in ['kp:sn2','kp:collision']:
            self.named('knowledge_points',k)
        self.entity('concept:particle-animation','concept',definition='Particle animation',kinds_json='["representation_method","motion"]')
        for kp,purpose in [('kp:sn2','show collision geometry'),('kp:collision','show collision geometry')]:
            self.put('knowledge_point_representations',knowledge_point_id=kp,concept_id='concept:particle-animation',context_key='geometry',context_json=json.dumps({'purpose':purpose}),verdict='conditional',rationale='Useful when spatial relation matters',curator='tester',evidence_set_id='ev:reason')
        self.assertEqual([], validate(self.db))
    ...
    def test_F_primitive_many_capabilities(self):
        ...
        for cap in ['cap:picking','cap:intersection']:
            self.named('tool_capabilities',cap)
            self.put('primitive_capabilities',tool_primitive_id='primitive:raycaster',capability_id=cap,scope_key='r180',evidence_set_id='ev:obs')
        self.assertEqual([], validate(self.db))

    def test_G_actual_tool_fact_and_recommendation_coexist(self):
        self.entity('example:actual','example')
        self.entity('concept:effect','concept',definition='Effect',kinds_json='["effect"]')
        self.entity('tool:three','tool',tool_subtype='library')
        self.put('entity_relations',from_entity_id='example:actual',to_entity_id='tool:three',predicate='built_with',evidence_set_id='ev:obs')
        self.put('entity_recommendations',from_entity_id='concept:effect',to_entity_id='tool:three',predicate='can_implement_with',context_key='web',context_json='{"purpose":"web implementation"}',verdict='suitable',rationale='A possible route',curator='tester',evidence_set_id='ev:reason',review_status='accepted')
        self.assertEqual([], validate(self.db))
...
class AdversarialGaps(Base):
    ...
    def test_01_blank_locator_should_not_publish_fact(self):
        self.db.execute("DELETE FROM evidence_set_sources WHERE evidence_set_id='ev:obs'")
        with self.assertRaises(sqlite3.IntegrityError):
            self.put('evidence_set_sources',evidence_set_id='ev:obs',source_id='src:web',locator='',stance='supports')
        self.entity('example:x','example'); self.entity('concept:x','concept')
        self.put('entity_relations',from_entity_id='example:x',to_entity_id='concept:x',predicate='demonstrates',evidence_set_id='ev:obs')
    ...
    def test_02_accepted_evidence_mutation_should_invalidate_review(self):
        # Once accepted, adding new contradictory material should require re-review or an immutable evidence revision.
        self.put('evidence_set_sources',evidence_set_id='ev:obs',source_id='src:web',locator='section-2',stance='contradicts')
        status=self.db.execute("SELECT review_status FROM evidence_sets WHERE id='ev:obs'").fetchone()[0]
    ...
    def test_04_accepted_representation_edit_should_reset_review(self):
        self.named('knowledge_points','kp:x'); self.entity('concept:repr','concept',definition='r',kinds_json='["representation_method"]')
        self.put('knowledge_point_representations',knowledge_point_id='kp:x',concept_id='concept:repr',context_key='c',context_json='{"purpose":"p1"}',verdict='suitable',rationale='r1',curator='tester',review_status='accepted')
        self.db.execute("UPDATE knowledge_point_representations SET verdict='avoid', rationale='changed' WHERE knowledge_point_id='kp:x'")
        status=self.db.execute("SELECT review_status FROM knowledge_point_representations WHERE knowledge_point_id='kp:x'").fetchone()[0]
    ...
    def test_05_entity_intent_requires_purpose_and_nonblank_rationale(self):
        self.entity('example:x','example')
        self.named('intents','intent:dramatic')
        with self.assertRaises(sqlite3.IntegrityError):
            self.put('entity_intents',entity_id='example:x',intent_id='intent:dramatic',context_key='x',context_json='{}',polarity='use_when',rationale='',curator='tester',review_status='accepted')
        self.put('entity_intents',entity_id='example:x',intent_id='intent:dramatic',context_key='x',context_json='{}',polarity='use_when',rationale='nonblank',curator='tester',review_status='accepted')
        errors=validate(self.db)
    ...
    def test_06_native_support_cannot_name_external_provider(self):
        ...
        self.named('tool_capabilities','cap:x')
        with self.assertRaises(sqlite3.IntegrityError):
            self.put('tool_capability_support',tool_entity_id='tool:a',capability_id='cap:x',scope_key='v1',mode='native',provider_tool_entity_id='tool:b',evidence_set_id='ev:obs')
    ...
    def test_08_accepted_intent_edit_should_reset_review(self):
        self.entity('example:x','example'); self.named('intents','intent:x')
        self.put('entity_intents',entity_id='example:x',intent_id='intent:x',context_key='web',context_json='{"purpose":"p1"}',polarity='use_when',rationale='r1',curator='tester',review_status='accepted')
        self.db.execute("UPDATE entity_intents SET polarity='avoid_when', rationale='changed' WHERE entity_id='example:x'")
        status=self.db.execute("SELECT review_status FROM entity_intents WHERE entity_id='example:x'").fetchone()[0]
    ...
    def test_11_hot_path_queries_use_reverse_indexes(self):
        queries={
            ...
            'entity_subjects_reverse': "SELECT entity_id FROM entity_subjects WHERE subject_id='subject:x'",
